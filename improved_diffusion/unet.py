...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.freeu_params = freeu_params
    ...
    def forward(self, x, timesteps, y=None):
        ...
        for flag, module in enumerate(self.output_blocks):
            hs_ = hs.pop()

            # --------------- FreeU code -----------------------
            # Only operate on the first two stages
            if flag <= 1 and self.freeu_params is not None:
                hidden_mean = h.mean(1).unsqueeze(1)
                B, C = hidden_mean.shape[:2]
                b, s = self.freeu_params[flag*2: flag*2+2]
